...
export function registerEditingTools(server: McpServer, bridge: HwpBridge, toolset: Toolset = 'standard'): void {
      ...
      try {
        ...
        for (const table of tables) {
          ...
          if (rowColCells.length > 0) {
            ...
            if (file_path) params.file_path = path.resolve(file_path);
        ...
        // Merge results
        const merged = { filled: 0, failed: 0, errors: [] as string[] };
        for (const r of results) {
          const d = r as { filled?: number; failed?: number; errors?: string[] };
          merged.filled += d.filled ?? 0;
          merged.failed += d.failed ?? 0;
          if (d.errors) merged.errors.push(...d.errors);
        }
  ...
  server.tool(
    ...
    async ({ find, replace, use_regex, case_sensitive }) => {
      ...
      try {
        ...
        if (case_sensitive === false) params.case_sensitive = false;
  ...
  server.tool(
    ...
    async ({ heading, body_text, body_style, skip_toc, occurrence }) => {
      try {
        ...
        const params: Record<string, unknown> = { heading, body_text };
        if (body_style) params.body_style = body_style;
        if (skip_toc !== undefined) params.skip_toc = skip_toc;
  ...
  server.tool(
    ...
    async ({ text, color, style, outline_level }) => {
      ...
      try {
        ...
        if (outline_level !== undefined) params.outline_level = outline_level;
  ...
  server.tool(
    ...
    async ({ align, line_spacing, line_spacing_type, space_before, space_after, indent, first_line_indent, left_margin, right_margin, page_break_before, keep_with_next, widow_orphan, line_wrap, snap_to_grid, auto_space_eAsian_eng, auto_space_eAsian_num, break_latin_word, heading_type, keep_lines_together, condense, border_left, border_right, border_top, border_bottom, border_color, border_shadowing, first_line_indent_hwpunit, hanging_indent, paragraph_heading_type, word_spacing, line_weight }) => {
      ...
      try {
        ...
        if (line_spacing !== undefined) style.line_spacing = line_spacing;
        if (line_spacing_type !== undefined) style.line_spacing_type = line_spacing_type;
        if (space_before !== undefined) style.space_before = space_before;
        if (space_after !== undefined) style.space_after = space_after;
        if (indent !== undefined) style.indent = indent;
        if (first_line_indent !== undefined) style.first_line_indent = first_line_indent;
        if (left_margin !== undefined) style.left_margin = left_margin;
        if (right_margin !== undefined) style.right_margin = right_margin;
        if (page_break_before !== undefined) style.page_break_before = page_break_before;
        if (keep_with_next !== undefined) style.keep_with_next = keep_with_next;
        if (widow_orphan !== undefined) style.widow_orphan = widow_orphan;
        if (line_wrap !== undefined) style.line_wrap = line_wrap;
        if (snap_to_grid !== undefined) style.snap_to_grid = snap_to_grid;
        if (auto_space_eAsian_eng !== undefined) style.auto_space_eAsian_eng = auto_space_eAsian_eng;
        if (auto_space_eAsian_num !== undefined) style.auto_space_eAsian_num = auto_space_eAsian_num;
        if (break_latin_word !== undefined) style.break_latin_word = break_latin_word;
        if (heading_type !== undefined) style.heading_type = heading_type;
        ...
        if (border_left !== undefined) style.border_left = border_left;
        if (border_right !== undefined) style.border_right = border_right;
        if (border_top !== undefined) style.border_top = border_top;
  ...
  server.tool(
    ...
    async ({ file_path, width, height, treat_as_char, embedded }) => {
      ...
      try {
        await bridge.ensureRunning();
        const params: Record<string, unknown> = { file_path: path.resolve(file_path) };
        if (width) params.width = width;
        if (height) params.height = height;
        if (treat_as_char !== undefined) params.treat_as_char = treat_as_char;
        if (embedded !== undefined) params.embedded = embedded;
  ...
  server.tool(
    ...
    async ({ table_index, start_row, start_col, end_row, end_col }) => {
      ...
      try {
        await bridge.ensureRunning();
        const params: Record<string, unknown> = { table_index };
        if (start_row !== undefined) params.start_row = start_row;
        if (start_col !== undefined) params.start_col = start_col;
        if (end_row !== undefined) params.end_row = end_row;
        if (end_col !== undefined) params.end_col = end_col;
  ...
  server.tool(
    ...
    '2D 배열 데이터로 새 표를 생성합니다. col_widths/row_heights 로 mm 단위 정밀 레이아웃. ' +
    ...
    async ({ data, header_style, header_bg_color, header_text_color, col_widths, row_heights, alignment, treat_as_char }) => {
      ...
      try {
        ...
        if (header_style) params.header_style = header_style;
        if (header_bg_color) params.header_bg_color = header_bg_color;
        if (header_text_color) params.header_text_color = header_text_color;
        if (col_widths) params.col_widths = col_widths;
        if (row_heights) params.row_heights = row_heights;
  ...
  server.tool(
    'hwp_insert_heading',
    '제목 텍스트를 삽입합니다 (H1~H9). 공문서 순번 체계의 대제목 등에 사용. numbering으로 자동 순번(텍스트 prefix)을 붙일 수 있습니다 (예: Ⅰ. 제목, 1. 제목, 가. 제목). v0.6.9+: auto_outline_level 옵션으로 ParaShape.OutlineLevel 자동 설정 → 한글 "개요 보기" + hwp_generate_toc 계층 인식 활성화. level 범위 1~6 → 1~9 확장.',
    {
    ...
    async ({ text, level, numbering, number, auto_outline_level, outline_level_only }) => {
      ...
      try {
        ...
        if (auto_outline_level !== undefined) params.auto_outline_level = auto_outline_level;
        if (outline_level_only !== undefined) params.outline_level_only = outline_level_only;
  ...
  server.tool(
    ...
    { output_path: z.string().describe('DOCX 저장 경로') },
    async ({ output_path }) => {
      if (!bridge.getCurrentDocument()) return { content: [{ type: 'text', text: JSON.stringify({ error: '열린 문서가 없습니다.' }) }], isError: true };
      try {
        await bridge.ensureRunning();
  ...
  server.tool(
    ...
    { output_path: z.string().describe('HTML 저장 경로') },
    async ({ output_path }) => {
      if (!bridge.getCurrentDocument()) return { content: [{ type: 'text', text: JSON.stringify({ error: '열린 문서가 없습니다.' }) }], isError: true };
      try {
        await bridge.ensureRunning();
  ...
  server.tool(
    'hwp_set_background_picture',
    '문서에 배경 이미지를 설정합니다. 워터마크로 활용 가능합니다.',
    { file_path: z.string().describe('배경 이미지 파일 경로') },
    async ({ file_path }) => {
      if (!bridge.getCurrentDocument()) return { content: [{ type: 'text', text: JSON.stringify({ error: '열린 문서가 없습니다.' }) }], isError: true };
      try {
        await bridge.ensureRunning();
  ...
  server.tool(
    'hwp_enter_table',
    '지정된 표에 진입하여 첫 셀에 커서를 위치시킵니다. (v0.6.8.1 + v0.7.3.1 path-based) ' +
    ...
    async ({ table_index, select_cell }) => {
      ...
      try {
        await bridge.ensureRunning();
        const params: Record<string, unknown> = { table_index };
        if (select_cell !== undefined) params.select_cell = select_cell;
  ...
  server.tool(
    ...
    { text: z.string().optional().describe('각주 내용 (생략 시 빈 각주)') },
    async ({ text }) => {
      if (!bridge.getCurrentDocument()) return { content: [{ type: 'text', text: JSON.stringify({ error: '열린 문서가 없습니다.' }) }], isError: true };
      try {
        await bridge.ensureRunning();
  ...
  server.tool(
    'hwp_insert_endnote',
    '현재 커서 위치에 미주를 삽입합니다.',
    { text: z.string().optional().describe('미주 내용') },
    async ({ text }) => {
      if (!bridge.getCurrentDocument()) return { content: [{ type: 'text', text: JSON.stringify({ error: '열린 문서가 없습니다.' }) }], isError: true };
      try {
        await bridge.ensureRunning();
  ...
  server.tool('hwp_extract_guide_text', '양식의 작성요령 박스 내용을 구조화 추출합니다 (v0.7.7 신규). 삭제 전에 호출하여 각 섹션별 요구사항(페이지 제한, 필수 기재, 형식 요구)을 보존. 결과를 AI 콘텐츠 생성 컨텍스트로 활용.', {
  }, async () => {
    if (!bridge.getCurrentDocument()) return { content: [{ type: 'text', text: JSON.stringify({ error: '열린 문서가 없습니다.' }) }], isError: true };
    try {
      await bridge.ensureRunning();
  ...
  server.tool('hwp_delete_guide_text', '양식의 작성요령/가이드 텍스트를 자동 삭제합니다. v0.7.5.4 업그레이드: scope="text"(기존 텍스트 리터럴), "table"(작성요령 박스 표 전체 삭제), "both"(둘 다). 공공기관 양식 작성 후 제출 전에 사용. 기본 scope="both" 권장. v0.7.7: extract_first=true 로 삭제 전 작성요령 내용 자동 추출.', {
    ...
    try {
      ...
      if (table_keywords) params.table_keywords = table_keywords;
      if (scope) params.scope = scope;
      if (extract_first !== undefined) params.extract_first = extract_first;
