...
export function registerEditingTools(server: McpServer, bridge: HwpBridge, toolset: Toolset = 'standard'): void {
  ...
  server.tool(
    ...
    '2D 배열 데이터로 새 표를 생성합니다. col_widths로 열 너비(mm), row_heights로 행 높이(mm)를 지정할 수 있습니다. 공문서 표 등 정밀한 레이아웃에 사용하세요.',
    {
  ...
  server.tool(
    'hwp_enter_table',
    '지정된 표(0-based 인덱스, 음수 가능)에 진입하여 첫 셀에 커서를 위치시킵니다. (v0.6.8.1 신규) 진입 후 hwp_navigate_cell, hwp_insert_row_at_cursor, hwp_insert_text, hwp_merge_current_selection 등으로 셀 단위 작업 가능. 작업 완료 후 hwp_exit_table로 명시적 탈출. WOW #4 시나리오 ("두 번째 표 마지막 행에 합계 추가")의 mcp 자동화 진입점. 안전망: 이미 다른 표 셀 안에 있으면 자동으로 먼저 탈출.',
    {
