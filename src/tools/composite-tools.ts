...
export function registerCompositeTools(server: McpServer, bridge: HwpBridge): void {
  ...
  async function runAutoFixLoop(opts: {
    ...
    saveFmt: 'HWPX' | 'HWP';
    isCancelled?: () => boolean;
    onIteration?: (entry: Record<string, unknown>) => void;
  }): Promise<{
    iterations: number;
    score_before: number;
    score_after: number;
    ...
    let score = 100;
    const recommendedFixes: Array<Record<string, unknown>> = [];
    try {
      const params: Record<string, unknown> = { file_path: opts.outputPath };
      if (opts.styleProfile) params.expected_profile = opts.styleProfile;
      const r = await bridge.send('validate_consistency', params, ANALYSIS_TIMEOUT);
      if (r.success && r.data) {
        const d = r.data as Record<string, unknown>;
        if (typeof d.consistency_score === 'number') score = d.consistency_score;
        // 발견된 deviations 를 recommended_fixes 로 반환 (Claude host 가 개별 수정)
        if (Array.isArray(d.format_deviations)) {
          for (const dev of d.format_deviations as Array<Record<string, unknown>>) {
  ...
  server.tool(
    ...
    async ({ template_path, data_list, output_dir }) => {
      ...
      try {
        ...
        if (output_dir) params.output_dir = path.resolve(output_dir);
  ...
  server.tool(
    ...
    async ({ file_path }) => {
      ...
      try {
        ...
        const analysis = response.data as AnalysisResult;
    ...
    async ({ file_path }) => {
      try {
        ...
        if (file_path) {
          ...
          text = (resp.data as { full_text?: string }).full_text || '';
        } else {
  ...
  server.tool(
    ...
    async ({ file_path }) => {
      ...
      try {
        ...
        const analysis = analysisResp.data as AnalysisResult;
        bridge.setCachedAnalysis(analysis);
        bridge.setCurrentDocument(resolved);
        ...
        const typePatterns: Array<[string, string[]]> = [
        ...
        if (emptyCells > 0) recommendations.push(`hwp_fill_table_cells 또는 hwp_smart_fill로 ${emptyCells}개 빈 셀 채우기`);
        if (emptyFields.length > 0) recommendations.push(`hwp_fill_fields로 ${emptyFields.length}개 빈 필드 채우기`);
        if (completionRate >= 90) recommendations.push('hwp_save_document로 저장');
        if (documentType.includes('사업계획서')) recommendations.push('fill_public_document 프롬프트로 공문서 표준 작성');
  ...
  server.tool(
    ...
    async ({ file_path: refPath, table_index, row_index }) => {
      ...
      try {
        ...
        const refData = refResp.data as Record<string, unknown>;

        // 헤더와 데이터 행 추출
        let headers: string[] = [];
        let dataRow: string[] = [];
        const format = refData.format as string;
        const ri = row_index ?? 0;

        if (format === 'csv') {
          headers = (refData.headers as string[]) || [];
          const data = (refData.data as string[][]) || [];
          dataRow = data[ri] || [];
        } else if (format === 'excel') {
          const sheets = (refData.sheets as Array<{ headers: string[]; data: string[][] }>) || [];
          if (sheets.length > 0) {
        ...
        const mapData = mapResp.data as { mappings: Array<{ tab: number; text: string; header: string; matched_label: string }>; unmapped: unknown[] };
  ...
  server.tool('hwp_batch_convert', '폴더 내 모든 HWP 파일을 지정 형식으로 일괄 변환합니다.', {
    ...
    try {
      await bridge.ensureRunning();
      const params: Record<string, unknown> = { input_dir: path.resolve(input_dir), output_format };
      if (output_dir) params.output_dir = path.resolve(output_dir);
  ...
  server.tool(
    ...
    async ({ dot_leader }) => {
      ...
      try {
        await bridge.ensureRunning();
        const params: Record<string, unknown> = {};
        if (dot_leader !== undefined) params.dot_leader = dot_leader;
  ...
  server.tool(
    ...
    async ({ tasks, months, month_label }) => {
      ...
      try {
        await bridge.ensureRunning();
        const params: Record<string, unknown> = { tasks, months };
        if (month_label) params.month_label = month_label;
  ...
  server.tool(
    ...
    async ({ file_path, field_types, output_path }) => {
      try {
        ...
        const doc = await readHwpxXml(resolved, 'Contents/section0.xml');
        const result = markFieldsForRecalc(doc, field_types as any);
        await writeHwpxXml(resolved, outResolved, 'Contents/section0.xml', doc);
  ...
  server.tool(
    ...
    async ({ file_path, max_depth }) => {
      if (!bridge.getCurrentDocument() && file_path) {
        try { await bridge.send('open_document', { file_path }); } catch {}
      }
      try {
        await bridge.ensureRunning();
        const params: Record<string, unknown> = { file_path };
  ...
  server.tool(
    ...
    async ({ file_path, user_request, reference_files, mode, constraints }) => {
      ...
      try {
        await bridge.ensureRunning();
        const params: Record<string, unknown> = { user_request };
        if (file_path) params.file_path = file_path;
        if (reference_files) params.reference_files = reference_files;
  ...
  server.tool(
    ...
    async (args) => {
      ...
      const loadCheckpoint = (): Record<string, unknown> => {
        if (!fs.existsSync(sessionPath)) return {};
        try { return JSON.parse(fs.readFileSync(sessionPath, 'utf8')); } catch { return {}; }
      };
      const saveCheckpoint = (extra: Record<string, unknown>): Record<string, unknown> => {
      ...
      try {
        ...
        const formFile = args.form_file ?? (cp.form_file as string | undefined);
        const outputPath = args.output_path
          ?? (cp.output_path as string | undefined)
          ?? (formFile ? formFile.replace(/(\.hwpx?)$/i, '_filled$1') : undefined);

        // ─── Phase 1: LEARN ───
        const runLearn = async (): Promise<Record<string, unknown>> => {
          if (!formFile) throw new Error('form_file required for learn phase');
          ...
          if (!openR.success) throw new Error(`open_document failed: ${openR.error}`);

          let formDetect: unknown = null;
          try {
            const r = await bridge.send('form_detect', {}, ANALYSIS_TIMEOUT);
            if (r.success) formDetect = r.data;
          } catch {}

          let structure: unknown = null;
          try {
            const r = await bridge.send('extract_template_structure', { file_path: formFile }, ANALYSIS_TIMEOUT);
            if (r.success) structure = r.data;
          } catch {}

          let patterns: unknown = null;
          try {
            const r = await bridge.send('analyze_writing_patterns', { file_path: formFile }, ANALYSIS_TIMEOUT);
            if (r.success) patterns = r.data;
          } catch {}

          // Enumerate tables and map cells
          let tablesInfo: Array<Record<string, unknown>> = [];
          let fieldsInfo: Array<Record<string, unknown>> = [];
          try {
            const r = await bridge.send('analyze_document', { file_path: formFile }, ANALYSIS_TIMEOUT);
            if (r.success && r.data) {
              const d = r.data as Record<string, unknown>;
              tablesInfo = Array.isArray(d.tables) ? d.tables as Array<Record<string, unknown>> : [];
              fieldsInfo = Array.isArray(d.fields) ? d.fields as Array<Record<string, unknown>> : [];
            }
          } catch {}

          const tableMaps: Array<Record<string, unknown>> = [];
          for (let i = 0; i < tablesInfo.length; i++) {
            try {
              const m = await bridge.send('map_table_cells', { table_index: i }, ANALYSIS_TIMEOUT);
              if (m.success && m.data) {
                tableMaps.push({ table_index: i, ...(m.data as Record<string, unknown>) });
              }
            } catch {}
          }
        ...
        // ─── Phase 2: PLAN ───
        const runPlan = async (): Promise<Record<string, unknown>> => {
          const learned = (cp.phase === 'learn' || cp.table_maps) ? cp : await runLearn();

          let estimate: Record<string, unknown> = {};
          try {
            const estParams: Record<string, unknown> = {
              user_request: args.user_request || '양식 채우기',
            };
            if (formFile) estParams.file_path = formFile;
            if (args.reference_file) estParams.reference_files = [args.reference_file];
          ...
          const suggestedFills: Array<Record<string, unknown>> = [];
          const tableMaps = (learned.table_maps as Array<Record<string, unknown>>) || [];
          for (const tm of tableMaps) {
            const cells = (tm.cell_map as Array<Record<string, unknown>>)
              || (tm.cells as Array<Record<string, unknown>>)
              || [];
        ...
        // ─── Phase 3: PREVIEW ───
        const runPreview = (): Record<string, unknown> => {
          return saveCheckpoint({
            phase: 'preview',
        ...
        // ─── Phase 4: FILL ───
        const runFill = async (): Promise<Record<string, unknown>> => {
          if (!args.confirm_fill) {
            return saveCheckpoint({ phase: 'fill', status: 'denied', reason: 'confirm_fill=false' });
          }
          if (!formFile) throw new Error('form_file missing in checkpoint');
          ...
          if (fs.existsSync(formFile)) {
            try { fs.copyFileSync(formFile, snapshotPath); } catch {}
          }
            ...
            try {
              ...
              if (refR.success && refR.data) {
                ...
                if (refData.format === 'hwp_structured' && Array.isArray(refData.tables)) {
                  const tables = refData.tables as Array<Record<string, unknown>>;
                  if (tables.length > 0) {
                    const firstTable = tables[0];
                    headers = (firstTable.headers as string[]) || [];
                    const dataRows = firstTable.data as Array<Array<unknown>> | undefined;
                ...
                if (headers.length === 0 && Array.isArray(refData.headers)) {
                  headers = refData.headers as string[];
                  const dataRows = refData.data as Array<Array<unknown>> | Array<Record<string, unknown>> | undefined;
                  if (Array.isArray(dataRows) && dataRows.length > 0) {
                    const first = dataRows[0];
                    row = Array.isArray(first) ? first.map(String) : Object.values(first).map(String);
                  }
                } else if (headers.length === 0 && Array.isArray(refData.sheets) && (refData.sheets as Array<Record<string, unknown>>).length > 0) {
                  const sheet0 = (refData.sheets as Array<Record<string, unknown>>)[0];
                  headers = (sheet0.headers as string[]) || [];
                  const dataRows = sheet0.data as Array<Array<unknown>> | undefined;
                  if (Array.isArray(dataRows) && dataRows.length > 0) {
                    row = dataRows[0].map(String);
                  }
                }

                const tableMaps = (cp.table_maps as Array<Record<string, unknown>>) || [];
                ...
                if (headers.length > 0 && tableMaps.length > 0) {
                  ...
                  const perTableResults: Array<Record<string, unknown>> = [];
                  for (const tm of tableMaps) {
                    const tblIdx = (tm.table_index as number) ?? 0;
                    try {
                      const m = await bridge.send('auto_map_reference',
                        { table_index: tblIdx, ref_headers: headers, ref_row: row },
                        ANALYSIS_TIMEOUT);
                      if (m.success && m.data) {
                        const mData = m.data as Record<string, unknown>;
                        const tblMappings = (mData.mappings as Array<Record<string, unknown>>) || [];
                        for (const mp of tblMappings) {
                          allMappings.push({ ...mp, table_index: tblIdx });
                        }
                        perTableResults.push({
                          table_index: tblIdx,
                          total_matched: (mData.total_matched as number) || 0,
                          unmapped: (mData.unmapped as Array<unknown>) || [],
                        });
                      }
                    } catch (e) {
                      perTableResults.push({
          ...
          if (args.table_cell_overrides) {
            for (const t of args.table_cell_overrides) {
              try {
                ...
                fillResults.push({ type: 'table_cell_overrides', table_index: t.table_index, ok: false, error: (e as Error).message });
              }
            }
          }
          ...
          if (refMappings && Array.isArray(refMappings.mappings)) {
            const overrideTabs = new Set<string>(
              (args.table_cell_overrides ?? [])
                .flatMap(t => t.cells
                  .filter(c => c.tab !== undefined)
                  .map(c => `${t.table_index}:${c.tab}`))
            );
            // Group by table_index → 각 테이블별로 smart_fill 호출
            const byTable = new Map<number, Array<{ tab: number; text: string }>>();
            for (const mp of refMappings.mappings as Array<Record<string, unknown>>) {
              const tIdx = (mp.table_index as number) ?? 0;
              const key = `${tIdx}:${mp.tab}`;
              if (overrideTabs.has(key)) continue;
              if (!byTable.has(tIdx)) byTable.set(tIdx, []);
              byTable.get(tIdx)!.push({ tab: mp.tab as number, text: mp.text as string });
            }
            for (const [tIdx, cells] of byTable) {
              ...
              try {
                ...
                fillResults.push({ type: 'ref_auto_map', table_index: tIdx, ok: false, error: (e as Error).message });
              }
            }
          }
          ...
          if (args.field_overrides && Object.keys(args.field_overrides).length > 0) {
            try {
              ...
              fillResults.push({ type: 'field_overrides', ok: false, error: (e as Error).message });
            }
          }

          // Save to output_path
          const saveFmt: 'HWPX' | 'HWP' = outputPath.toLowerCase().endsWith('.hwpx') ? 'HWPX' : 'HWP';
          ...
          let autoVerify: Record<string, unknown> | null = null;
          if (saveR.success) {
            try {
              const verifyResult = await runVerify();
              autoVerify = (verifyResult.verify as Record<string, unknown>) || null;
            } catch (e) {
              autoVerify = { error: (e as Error).message };
            }
          }
        ...
        // ─── Phase 5: VERIFY ───
        const runVerify = async (): Promise<Record<string, unknown>> => {
          ...
          let wcData: Record<string, unknown> | null = null;
          try {
            const r = await bridge.send('word_count', {}, ANALYSIS_TIMEOUT);
            if (r.success && r.data) wcData = r.data as Record<string, unknown>;
          } catch {}
          ...
          try {
            ...
            if (r.success && r.data) {
              const d = r.data as Record<string, unknown>;
              const txt = (d.text as string) || (d.full_text as string) || '';
          ...
          try {
            const params: Record<string, unknown> = { file_path: outputPath };
            if (cp.writing_patterns) params.expected_profile = cp.writing_patterns;
            const r = await bridge.send('validate_consistency', params, ANALYSIS_TIMEOUT);
            if (r.success && r.data) {
              const d = r.data as Record<string, unknown>;
              if (typeof d.consistency_score === 'number') consistencyScore = d.consistency_score;
            }
          } catch {}

          let privacy: Record<string, unknown> | null = null;
          try {
            const r = await bridge.send('privacy_scan', { file_path: outputPath }, ANALYSIS_TIMEOUT);
            if (r.success && r.data) privacy = r.data as Record<string, unknown>;
          } catch {}
          ...
          let verify5Stage: Record<string, unknown> | null = null;
          try {
            ...
            if (r.success && r.data) verify5Stage = r.data as Record<string, unknown>;
          } catch {}
        ...
        // ─── Phase 6: AUTO_FIX ───
        const runAutoFixPhase = async (): Promise<Record<string, unknown>> => {
          if (!outputPath) throw new Error('output_path missing');
          const saveFmt: 'HWPX' | 'HWP' = outputPath.toLowerCase().endsWith('.hwpx') ? 'HWPX' : 'HWP';
          const loopResult = await runAutoFixLoop({
            outputPath,
            styleProfile: cp.writing_patterns as Record<string, unknown> | undefined,
        ...
        // ─── Rollback ───
        const runRollback = (): Record<string, unknown> => {
          if (!formFile) throw new Error('form_file missing');
          if (!fs.existsSync(snapshotPath)) throw new Error('no rollback snapshot — fill phase 를 먼저 실행해야 합니다');
        ...
        if (args.phase === 'learn') result = await runLearn();
        else if (args.phase === 'plan') result = await runPlan();
        else if (args.phase === 'preview') result = runPreview();
        else if (args.phase === 'fill') result = await runFill();
        else if (args.phase === 'verify') result = await runVerify();
        else if (args.phase === 'auto_fix') result = await runAutoFixPhase();
        else if (args.phase === 'rollback') result = runRollback();
        ...
        saveCheckpoint({ phase: args.phase, status: 'failed', error: (err as Error).message });
        return { content: [{ type: 'text', text: JSON.stringify({ error: (err as Error).message, session_id: sid }) }], isError: true };
      }
    }
  );
  ...
  server.tool(
    ...
    async (args) => {
      ...
      const pushLog = (step: string, data: Record<string, unknown>) => log.push({ step, elapsed_ms: Date.now() - startTime, ...data });
      ...
      try {
        ...
        pushLog('open_document', { ok: openR.success, pages: (openR.data as Record<string, unknown>)?.pages });
        if (!openR.success) throw new Error(`open failed: ${openR.error}`);

        // Step 2: Detect document type — hint 있으면 type_override 로 프리셋 강제 (v0.7.9 fix)
        let docType: string = args.doc_type_hint || 'general';
        let preset: Record<string, unknown> | null = null;
        try {
          const detectParams: Record<string, unknown> = {};
          if (args.doc_type_hint) detectParams.type_override = args.doc_type_hint;
          const r = await bridge.send('detect_document_type', detectParams, ANALYSIS_TIMEOUT);
          if (r.success && r.data) {
            const d = r.data as Record<string, unknown>;
            if (!args.doc_type_hint) docType = (d.type as string) || 'general';
            preset = (d.recommended_preset as Record<string, unknown>) || null;
            pushLog('detect_document_type', { type: docType, confidence: d.confidence });
          }
        } catch (e) { pushLog('detect_document_type', { error: (e as Error).message }); }

        // Step 3: Snapshot template style (optional, 실패해도 continue)
        try {
          const r = await bridge.send('snapshot_template_style', {}, ANALYSIS_TIMEOUT);
          if (r.success && r.data) {
            pushLog('snapshot_template_style', { snapshot_id: (r.data as Record<string, unknown>).snapshot_id });
          }
        } catch (e) { pushLog('snapshot_template_style', { error: (e as Error).message }); }

        // Step 4: Extract + Delete guide text (v0.7.7: extract_first)
        const deleteGuides = args.delete_guides ?? true;
        let extractedGuides: Array<Record<string, unknown>> = [];
        if (deleteGuides) {
          try {
            const r = await bridge.send('delete_guide_text', { scope: 'both', extract_first: true }, ANALYSIS_TIMEOUT);
            if (r.success && r.data) {
              const d = r.data as Record<string, unknown>;
              extractedGuides = (d.extracted_guides as Array<Record<string, unknown>>) || [];
              pushLog('delete_guide_text', { ...d, extracted_guide_count: extractedGuides.length });
            }
          } catch (e) { pushLog('delete_guide_text', { error: (e as Error).message }); }
        }

        // Step 5: Fill table cells (if provided)
        // fill_by_tab 을 각 table 별로 반복 호출 (composite 내부 분할)
        const tableFillResults: Array<Record<string, unknown>> = [];
        if (args.table_cell_overrides && args.table_cell_overrides.length > 0) {
          for (const tbl of args.table_cell_overrides) {
            try {
              ...
              if (r.success && r.data) {
                tableFillResults.push({ table_index: tbl.table_index, ...(r.data as Record<string, unknown>) });
              } else {
                tableFillResults.push({ table_index: tbl.table_index, error: r.error });
              }
            } catch (e) {
              tableFillResults.push({ table_index: tbl.table_index, error: (e as Error).message });
            }
          }
        ...
        const bodyResults: Array<Record<string, unknown>> = [];
        if (args.body_fills && args.body_fills.length > 0) {
          ...
          for (const fill of args.body_fills) {
            try {
              ...
              bodyResults.push({ heading: fill.heading, status: 'error', error: (e as Error).message });
            }
          }
        ...
        const saveFmt: 'HWPX' | 'HWP' = outPath.toLowerCase().endsWith('.hwpx') ? 'HWPX' : 'HWP';
        ...
        if (!saveR.success) throw new Error(`save failed: ${saveR.error}`);
        ...
        if (args.run_verify !== false) {
          ...
          } catch (e) { pushLog('verify_5stage', { error: (e as Error).message }); }
        }
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              output_path: outPath,
              doc_type: docType,
              preset_used: preset ? ((preset as Record<string, unknown>).description || docType) : 'none',
    ...
    async (args) => {
      ...
      try {
        ...
        try {
          ...
          if (r.success && r.data) {
            const d = r.data as Record<string, unknown>;
            docType = (d.type as string) || 'general';
            preset = (d.recommended_preset as Record<string, unknown>) || null;
            pushLog('detect_document_type', { type: docType, confidence: d.confidence });
          }
        } catch (e) { pushLog('detect_document_type', { error: (e as Error).message }); }
        ...
        let templateSections: Array<Record<string, unknown>> = [];
        try {
          const r = await bridge.send('extract_template_structure', { file_path: formPath }, ANALYSIS_TIMEOUT);
          if (r.success && r.data) {
            const d = r.data as Record<string, unknown>;
            templateSections = (d.sections as Array<Record<string, unknown>>) || [];
            pushLog('extract_template_structure', { sections: templateSections.length });
          }
        } catch (e) { pushLog('extract_template_structure', { error: (e as Error).message }); }

        // Step 4: Snapshot template style
        let snapshotId = '';
        try {
          const r = await bridge.send('snapshot_template_style', {}, ANALYSIS_TIMEOUT);
          if (r.success && r.data) snapshotId = String((r.data as Record<string, unknown>).snapshot_id || '');
          pushLog('snapshot_template_style', { snapshot_id: snapshotId });
        } catch (e) { pushLog('snapshot_template_style', { error: (e as Error).message }); }

        // Step 5: Extract guide text (before deletion)
        let guideConstraints: Array<Record<string, unknown>> = [];
        try {
          const r = await bridge.send('extract_guide_text', {}, ANALYSIS_TIMEOUT);
          if (r.success && r.data) {
            guideConstraints = ((r.data as Record<string, unknown>).guides as Array<Record<string, unknown>>) || [];
            pushLog('extract_guide_text', { count: guideConstraints.length });
          }
        } catch (e) { pushLog('extract_guide_text', { error: (e as Error).message }); }
        ...
        if (args.reference_files && args.reference_files.length > 0) {
          ...
          for (const refFile of args.reference_files) {
            try {
              ...
              if (r.success && r.data) {
                const d = r.data as Record<string, unknown>;
                if (d.full_text) allTexts.push(String(d.full_text));
                else if (d.text) allTexts.push(String(d.text));
                if (d.tables) allTables.push(...(d.tables as Array<Record<string, unknown>>));
              }
            } catch (e) { pushLog('read_reference', { file: refFile, error: (e as Error).message }); }
          }
        ...
        let sectionMappings: Array<Record<string, unknown>> = [];
        try {
          ...
          if (r.success && r.data) {
            const d = r.data as Record<string, unknown>;
            sectionMappings = (d.section_mappings as Array<Record<string, unknown>>) || [];
            pushLog('map_reference_to_sections', { total: d.total_sections, with_data: d.sections_with_data });
          }
        } catch (e) { pushLog('map_reference_to_sections', { error: (e as Error).message }); }
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              form_file: formPath,
              doc_type: docType,
              preset: preset ? { description: (preset as Record<string, unknown>).description } : null,
        ...
        return {
          content: [{ type: 'text', text: JSON.stringify({ error: (err as Error).message, log }) }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    ...
    async ({ file_path, path: cellPath, find, replace, output_path }) => {
      try {
        ...
        const doc = await readHwpxXml(resolved, 'Contents/section0.xml');
        const result = replaceInNestedTable(doc, cellPath as any, find, replace);
        await writeHwpxXml(resolved, outResolved, 'Contents/section0.xml', doc);
  ...
  server.tool(
    ...
    async ({ file_path, expected_profile }) => {
      ...
      try {
        await bridge.ensureRunning();
        const params: Record<string, unknown> = { file_path };
        if (expected_profile) params.expected_profile = expected_profile;
        const r = await bridge.send('validate_consistency', params, ANALYSIS_TIMEOUT);
        if (!r.success) return { content: [{ type: 'text', text: JSON.stringify({ error: r.error }) }], isError: true };
        return { content: [{ type: 'text', text: JSON.stringify(r.data) }] };
      } catch (err) { return { content: [{ type: 'text', text: JSON.stringify({ error: (err as Error).message }) }], isError: true }; }
  ...
  function readConfig(): { reference_policy: typeof DEFAULT_POLICY } {
    try {
      ...
      try {
        ...
        if (mode === 'list') {
          ...
          const sessions = files.map(f => {
            ...
            } catch { return { session_id: f.replace('.json', ''), error: 'parse_failed' }; }
          });
      ...
      try {
        ...
        if (mode === 'search') {
          ...
          const scored = items.map(item => {
            ...
            if (item.last_used) score += 0.5;
            return { ...item, _score: score };
          }).filter(x => x._score > 0).sort((a, b) => (b._score as number) - (a._score as number));
  ...
  server.tool(
    ...
    async ({ file_path, checks, auto_fix, expected_profile }) => {
      try {
        ...
        const auto_fixed: Array<Record<string, unknown>> = [];
        const requires_manual: Array<Record<string, unknown>> = [];
        ...
        if (checkSet.has('privacy')) {
          ...
          if (r.success && r.data) {
            const data = r.data as Record<string, unknown>;
            const findings = (data.findings as unknown[]) || (data.matches as unknown[]) || [];
            for (const f of findings) {
              issues.push({ check: 'privacy', severity: 'high', detail: f });
              requires_manual.push({ check: 'privacy', detail: f, reason: '개인정보는 사람 확인 필수' });
            }
            if (findings.length > 0) score_before = Math.min(score_before, Math.max(0, 100 - findings.length * 10));
          }
        }
            ...
            try {
              ...
              if (fixR.success) {
                ...
                if (reR.success && reR.data) {
                  const newScore = (reR.data as any).consistency_score;
                  if (typeof newScore === 'number') score_after = newScore;
                }
              } else {
                auto_fixed.push({ check: 'consistency', method: 'apply_style_profile', error: fixR.error });
              }
            } catch (e) {
              auto_fixed.push({ check: 'consistency', error: (e as Error).message });
            }
  ...
  server.tool(
    ...
    async ({ result_path, template_path, weight }) => {
      try {
        ...
        const tData = (tStruct.data || {}) as Record<string, unknown>;
        const rData = (rStruct.data || {}) as Record<string, unknown>;
        const tSections = (tData.sections as Array<Record<string, unknown>>) || [];
        const rSections = (rData.sections as Array<Record<string, unknown>>) || [];
        ...
        if (result_path !== template_path) {
          try {
            ...
            const tStr = String((tText.data as any)?.text || '').trim();
            const rStr = String((rText.data as any)?.text || '').trim();
            if (tStr && rStr) {
              const tokenize = (s: string) => new Set(s.split(/\s+|[.,!?;:()\[\]{}<>'"`~\-_=+|\\\/]+/).filter(w => w.length >= 2));
        ...
        const overall_score = Math.round(
          format_score * w.format + structure_score * w.structure + content_score * w.content
        );
  ...
  server.tool(
    ...
    async ({ session_id }) => {
      try {
        ...
        const raw = JSON.parse(fs.readFileSync(filePath, 'utf8'));
        const total = (raw.sections_total as unknown[] || []).length;
        const done = (raw.sections_done as unknown[] || []).length;
        return { content: [{ type: 'text', text: JSON.stringify({
          ok: true,
          session_id,
  ...
  server.tool(
    ...
    async (args) => {
      ...
      try {
        ...
        let estimate: Record<string, unknown> = {};
        try {
          const estParams: Record<string, unknown> = { user_request: args.user_request };
          if (resolvedTemplate) estParams.file_path = resolvedTemplate;
          if (args.reference_files) estParams.reference_files = args.reference_files;
          const r = await bridge.send('estimate_workload', estParams, ANALYSIS_TIMEOUT);
          if (r.success && r.data) estimate = r.data as Record<string, unknown>;
        } catch {}

        // Phase 2: template structure + writing patterns (template 있을 때만)
        let templateStructure: Record<string, unknown> | null = null;
        let writingPatterns: Record<string, unknown> | null = null;
        if (resolvedTemplate) {
          try {
            const r = await bridge.send('extract_template_structure', { file_path: resolvedTemplate }, ANALYSIS_TIMEOUT);
            if (r.success && r.data) templateStructure = r.data as Record<string, unknown>;
          } catch {}
          try {
            const r = await bridge.send('analyze_writing_patterns', { file_path: resolvedTemplate }, ANALYSIS_TIMEOUT);
            if (r.success && r.data) writingPatterns = r.data as Record<string, unknown>;
          } catch {}
        }

        // Phase 3: derive section skeleton
        const tplSections = (templateStructure?.sections as Array<{ title?: string; level?: number }> | undefined) || [];
        ...
        const sectionsSkel: Array<Record<string, unknown>> = tplSections.length > 0
  ...
  server.tool(
    ...
    async (args) => {
      ...
      const preserveTemplateStyle = (args as Record<string, unknown>)['preserve_template_style'] as boolean | undefined ?? true;
      ...
      let hSections: Array<{ title: string; content: string; outline_level?: number }> | undefined =
        rawArgs['sections'] as Array<{ title: string; content: string; outline_level?: number }> | undefined;
      let hTables: Array<{ caption?: string; data: string[][] }> | undefined =
        rawArgs['tables'] as Array<{ caption?: string; data: string[][] }> | undefined;
      let hStyleProfile: unknown = rawArgs['style_profile'];
      let hOutputPath: string | undefined = rawArgs['output_path'] as string | undefined;
      let hTemplatePath: string | undefined = rawArgs['template_path'] as string | undefined;
      const templateId: string | undefined = rawArgs['template_id'] as string | undefined;
        ...
        try {
          ...
          if (fs.existsSync(planPath)) {
            ...
            if ((!hSections || hSections.length === 0) && Array.isArray(plan.sections)) {
              hSections = (plan.sections as Array<Record<string, unknown>>)
                .filter((s) => typeof s.content === 'string' && (s.content as string).trim().length > 0)
                .map((s) => ({
                  title: String(s.title || ''),
                  content: String(s.content || ''),
                  outline_level: typeof s.outline_level === 'number' ? (s.outline_level as number) : undefined,
                }));
            }
            if ((!hTables || hTables.length === 0) && Array.isArray(plan.tables)) {
              hTables = (plan.tables as Array<Record<string, unknown>>)
                .filter((t) => Array.isArray(t.data) && (t.data as unknown[]).length > 0)
                .map((t) => ({ caption: t.caption as string | undefined, data: t.data as string[][] }));
            }
            if (!hStyleProfile && plan.style_profile) hStyleProfile = plan.style_profile;
            if (!hOutputPath && plan.output_path) hOutputPath = String(plan.output_path);
            if (!hTemplatePath && plan.template_path) hTemplatePath = String(plan.template_path);
          }
        } catch {}
      }
      ...
      const sections: Array<{ title: string; content: string; outline_level?: number }> = hSections;
      const tables: Array<{ caption?: string; data: string[][] }> | undefined = hTables;
      const styleProfile: unknown = hStyleProfile;
      const outputPath: string = hOutputPath;
      const templatePath: string | undefined = hTemplatePath;
      ...
      const saveSession = (extra: Record<string, unknown> = {}) => {
        let existing: Record<string, unknown> = {};
        if (fs.existsSync(sessionPath)) {
          try { existing = JSON.parse(fs.readFileSync(sessionPath, 'utf8')); } catch {}
        }
        const merged = {
          ...existing,
          session_id: sid,
      ...
      const isCancelled = (): boolean => {
        ...
        try { return !!JSON.parse(fs.readFileSync(sessionPath, 'utf8')).cancelled; } catch { return false; }
      };
      const recordStep = (name: string, ok: boolean, detail?: unknown) => {
        stepsDone++;
      ...
      try {
        ...
        try {
          ...
          if (templatePath) estParams.file_path = templatePath;
          const r = await bridge.send('estimate_workload', estParams, ANALYSIS_TIMEOUT);
          if (r.success) estimate = r.data as Record<string, unknown>;
        } catch {}
        ...
        try {
          ...
          const allSectionsVerified = stepLog
            ...
            .every(s => ((s as { detail?: { body_verified?: boolean } }).detail)?.body_verified === true);
          ...
          if ((e as Error).message.includes('not verified')) throw e;
        ...
        let autoFixLog: Array<Record<string, unknown>> = [];
        let autoFixStopped = 'skipped';
        try {
          const loopResult = await runAutoFixLoop({
            outputPath,
            styleProfile: autoFix ? (styleProfile as Record<string, unknown> | undefined) : undefined,
          ...
          recordStep('validate_consistency', false, (e as Error).message);
        }
        ...
        } catch (e) { recordStep('verify_layout', false, (e as Error).message); }
