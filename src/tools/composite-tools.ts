...
export function registerCompositeTools(server: McpServer, bridge: HwpBridge): void {
  ...
  server.tool(
    ...
    '문서 자동 생성 12단계 파이프라인. (v0.7.2.4 신규) sections[]/tables[]를 받아 template 기반으로 작성→스타일→TOC→검증→저장→PDF→layout 검증까지 일괄 실행. mode=plan은 estimate만 반환, mode=execute는 실제 파이프 실행. 각 step마다 session_state 자동 save + cancel 체크.',
    {
