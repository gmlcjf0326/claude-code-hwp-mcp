...
function listHwpFiles(directory: string, recursive: boolean): FileInfo[] {
  ...
  function scan(dir: string, depth: number): void {
    ...
    for (const entry of entries) {
      ...
      if (!HWP_EXTENSIONS.has(ext)) continue;
...
export function registerDocumentTools(server: McpServer, bridge: HwpBridge): void {
    ...
    async ({ path: savePath, format }) => {
      ...
      if (!saveFormat) {
        ...
        if (['hwp', 'hwpx', 'pdf', 'docx'].includes(ext)) {
          saveFormat = ext as 'hwp' | 'hwpx' | 'pdf' | 'docx';
        } else {
          saveFormat = 'hwp';
        }
      }
