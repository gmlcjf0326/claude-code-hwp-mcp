...
async function ensureAnalysis(bridge: HwpBridge, filePath?: string): Promise<AnalysisResult> {
  ...
  const cached = bridge.getCachedAnalysis() as AnalysisResult | null;
  const current = bridge.getCurrentDocument();

  // P1 #8: 캐시가 현재 열린 문서와 일치하는 경우에만 반환
  if (cached && current && cached.file_path === current) return cached;
...
export function registerAnalysisTools(server: McpServer, bridge: HwpBridge, toolset: Toolset = 'standard'): void {

  server.tool(
    'hwp_analyze_document',
  ...
  server.tool(
    ...
    async ({ filter, max_visits }) => {
      try {
        ...
        if (!response.success) throw new Error(response.error ?? 'list_controls 실패');
  ...
  server.tool(
    ...
    async ({ table_index, sample_tabs }) => {
      ...
      try {
        ...
        if (sample_tabs) params.sample_tabs = sample_tabs;
  ...
  server.tool(
    ...
    async ({ file_path, max_chars }) => {
      ...
      try {
        await bridge.ensureRunning();
        const params: Record<string, unknown> = { file_path: resolved };
        if (max_chars) params.max_chars = max_chars;
      ...
      try {
        ...
        for (const table of tables) {
          ...
          if (table.headers && table.headers.length > 0) {
            // 헤더에서 빈 문자열을 '-'로 대체
            const headers = table.headers.map((h: string) => h.replace(/\r?\n/g, ' ').trim() || '-');
  ...
  server.tool(
    ...
    async ({ page }) => {
      ...
      try {
        ...
        const analysis = response.data as { full_text: string; pages: number };
  ...
  server.tool(
    ...
    async ({ output_dir, pages_per_split }) => {
      ...
      try {
        await bridge.ensureRunning();
        const resolved = path.resolve(output_dir);
        const params: Record<string, unknown> = { output_dir: resolved };
        if (pages_per_split) params.pages_per_split = pages_per_split;
  ...
  server.tool(
    ...
    {},
    async () => {
      if (!bridge.getCurrentDocument()) return { content: [{ type: 'text', text: JSON.stringify({ error: '열린 문서가 없습니다. hwp_open_document 로 양식 파일을 먼저 여세요.' }) }], isError: true };
      try {
        await bridge.ensureRunning();
  ...
  server.tool(
    ...
    async ({ file_path, expected_chars, expected_text_snippet, run_layout }) => {
      try {
        ...
        if (expected_chars !== undefined) params.expected_chars = expected_chars;
        if (expected_text_snippet !== undefined) params.expected_text_snippet = expected_text_snippet;
        if (run_layout !== undefined) params.run_layout = run_layout;
  ...
  server.tool(
    ...
    async (params) => {
      ...
      try {
        await bridge.ensureRunning();
        const r = await bridge.send('set_page_setup', params as Record<string, unknown>);
        if (!r.success) return { content: [{ type: 'text', text: JSON.stringify({ error: r.error }) }], isError: true };
        return { content: [{ type: 'text', text: JSON.stringify(r.data) }] };
      } catch (err) { return { content: [{ type: 'text', text: JSON.stringify({ error: (err as Error).message }) }], isError: true }; }
    }
  ...
  server.tool(
    ...
    async (params) => {
      ...
      try {
        await bridge.ensureRunning();
        const r = await bridge.send('set_cell_property', params as Record<string, unknown>);
        if (!r.success) return { content: [{ type: 'text', text: JSON.stringify({ error: r.error }) }], isError: true };
        return { content: [{ type: 'text', text: JSON.stringify(r.data) }] };
      } catch (err) { return { content: [{ type: 'text', text: JSON.stringify({ error: (err as Error).message }) }], isError: true }; }
    }
  ...
  server.tool(
    ...
    async (params) => {
      ...
      try {
        await bridge.ensureRunning();
        const r = await bridge.send('insert_textbox', params as Record<string, unknown>);
        if (!r.success) return { content: [{ type: 'text', text: JSON.stringify({ error: r.error }) }], isError: true };
        return { content: [{ type: 'text', text: JSON.stringify(r.data) }] };
      } catch (err) { return { content: [{ type: 'text', text: JSON.stringify({ error: (err as Error).message }) }], isError: true }; }
    }
  ...
  server.tool(
    ...
    async (params) => {
      ...
      try {
        await bridge.ensureRunning();
        const r = await bridge.send('draw_line', params as Record<string, unknown>);
        if (!r.success) return { content: [{ type: 'text', text: JSON.stringify({ error: r.error }) }], isError: true };
        return { content: [{ type: 'text', text: JSON.stringify(r.data) }] };
      } catch (err) { return { content: [{ type: 'text', text: JSON.stringify({ error: (err as Error).message }) }], isError: true }; }
    }
