...
export function registerResources(server: McpServer, bridge: HwpBridge): void {
  ...
  server.resource(
    ...
    async (uri) => {
      const analysis = bridge.getCachedAnalysis() as AnalysisResult | null;
      if (!analysis) {
        return {
          contents: [{
            uri: uri.href,
  ...
  server.resource(
    'table',
    new ResourceTemplate('hwp://tables/{index}', {
      list: async () => {
        const analysis = bridge.getCachedAnalysis() as AnalysisResult | null;
