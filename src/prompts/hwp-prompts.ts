...
export function registerPrompts(server: McpServer): void {
  ...
  server.prompt(
    'fill_document',
    'HWP 문서의 빈 필드와 표 셀을 사전질문→분석→미리보기→채우기→검증 흐름으로 채웁니다.',
    {
...
주의: 각 파일 처리 후 반드시 hwp_close_document로 닫은 후 다음 파일 진행`,
  ...
  server.prompt(
    'fill_public_document',
    '공공기관 사업계획서/신청서 작성. 기본: 빠른 모드(2개 질문). context에 "--상세" 추가 시 상세 모드(5개 질문+미리보기).',
    {
      file_path: z.string().describe('HWP 파일 경로'),
...
- 관인: 발신명의 마지막 글자가 인영 가운데`,
  ...
  server.prompt(
    'review_document',
    '작성 완료된 문서를 검토하고 수정점을 제안합니다. 공문서 표준 준수, 개인정보, 빈 항목, 내용 일관성을 점검합니다.',
    {
      file_path: z.string().describe('HWP 파일 경로'),
    },
    ({ file_path }) => ({
...
수정이 필요한 부분은 구체적인 위치(표 번호, 셀)와 수정 방법을 안내하세요.`,
  ...
  server.prompt(
    ...
    '엑셀/CSV 데이터로 HWP 양식을 자동으로 채웁니다. "엑셀로 양식 채워줘" 요청에 사용.',
    {
      file_path: z.string().describe('HWP 파일 경로'),
...
- 서식: 제목 Bold 22pt, 본문 11pt`,
  ...
  server.prompt(
    'write_document',
    '양식을 참고하여 새 문서를 처음부터 작성합니다. 양식 서식 추출 → 사전 질문 → 내용 작성 → 저장.',
    {
