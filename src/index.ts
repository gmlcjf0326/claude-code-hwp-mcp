...
const toolset = toolsetArg?.split('=')[1]
  ...
  ?? 'standard';
const validToolsets = ['minimal', 'standard', 'full'] as const;
type Toolset = typeof validToolsets[number];
const resolvedToolset: Toolset = validToolsets.includes(toolset as Toolset)
  ? (toolset as Toolset)
  : 'standard';
...
bridge.checkPrerequisites().then(prereq => {
  if (!prereq.ok) {
    ...
    if (!prereq.os.ok) console.error(`  ❌ ${prereq.os.error}`);
    if (!prereq.python.found) console.error('  ❌ Python 미설치 → https://www.python.org/downloads/ (PATH 추가 필수)');
    else if (!prereq.pyhwpx.found) console.error('  ❌ pyhwpx 미설치 → pip install pyhwpx');
    else if (!prereq.hwp.found) console.error('  ❌ 한글(HWP) 미설치 → 한컴오피스 설치 필요');
    console.error('  💡 자세한 진단: hwp_check_setup 도구를 호출하세요');
  } else {
    console.error(`[HWP MCP] ✅ 환경 준비 완료 (Python ${prereq.python.version})`);
  }
}).catch(() => { /* 진단 실패는 무시 — 서버는 계속 동작 */ });
