...
export class HwpBridge {
   ...
   */
  private findPython(): { exe: string; preArgs: string[] } {
  ...
  private async firstRunSetup(): Promise<void> {
    ...
    let prereq: PrerequisiteResult;
    try {
      prereq = await this.checkPrerequisites();
    } catch (err) {
      console.error(`[HWP Bridge] First-run checkPrerequisites 실패: ${(err as Error).message}`);
    ...
    if (pyhwpxMissing) missingList.push('pyhwpx');
    if (prereq.deps?.missingCore) missingList.push(...prereq.deps.missingCore);
  ...
  private writeSentinel(sentinelPath: string, data: Record<string, unknown>): void {
    try {
      ...
      console.error(`[HWP Bridge] Sentinel 기록 실패: ${(err as Error).message}`);
    }
  }

  // ── v0.7.4.6: 의존성 자동 설치 ──
  async installDeps(opts: { mode?: 'all' | 'core_only'; timeoutMs?: number } = {}): Promise<InstallDepsResult> {
    const { exe: pythonExe, preArgs } = this.findPython();
  ...
  async checkPrerequisites(): Promise<PrerequisiteResult> {
    ...
    try {
      ...
      let pythonGuide: string | undefined;
      if (isStorePython) {
    ...
    try {
      ...
      const d = JSON.parse(stdout.trim()) as Record<string, boolean>;
      const core = {
